return {
	...
	},
	{
		...
		config = function()
			-- Use the modern vim.lsp.config API if available, fallback to lspconfig
			local use_modern_api = vim.fn.has("nvim-0.11") == 1 and vim.lsp.config ~= nil
			local lspconfig = not use_modern_api and require("lspconfig") or nil
			...
			local function setup_lsp_keymaps(bufnr)
				...
				vim.keymap.set(
					...
					vim.tbl_extend("force", opts, { desc = "Show diagnostic error messages" })
				)
				vim.keymap.set(
					"n",
					"[d",
					vim.diagnostic.goto_prev,
					vim.tbl_extend("force", opts, { desc = "Go to previous diagnostic message" })
				)
				vim.keymap.set(
					"n",
					"]d",
					vim.diagnostic.goto_next,
					vim.tbl_extend("force", opts, { desc = "Go to next diagnostic message" })
				)
				vim.keymap.set(
					"n",
					"<leader>q",
				...
				for _, file in pairs(files) do
					if file:match("%.lua$") then
						...
						if ok and type(server_config) == "table" and server_config.name then
							...
							-- Setup server using appropriate API
							local setup_ok, setup_err = pcall(function()
								if use_modern_api then
									-- Use modern vim.lsp.config API
									vim.lsp.config[server_config.name] = final_config
								else
									-- Use legacy lspconfig
									lspconfig[server_config.name].setup(final_config)
								end
							end)
							if not setup_ok then
								vim.notify(
									string.format("Failed to setup LSP server '%s': %s", server_config.name, setup_err),
									vim.log.levels.WARN
								)
							end
						elseif not ok then
