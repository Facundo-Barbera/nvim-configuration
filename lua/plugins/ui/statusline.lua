	...
	config = function()
		...
		ins_left({
			function()
				...
				-- Add Python venv info if available and we're in a Python file
				if buf_ft == "python" then
					local ok, venv = pcall(require, "core.venv")
					if ok then
						local venv_info = venv.get_venv_info()
						if venv_info and venv_info.is_active then
							local venv_name = vim.fn.fnamemodify(venv_info.venv_path, ":t")
		...
		ins_left({
			function()
				...
				local lang = vim.opt_local.spelllang:get()[1] or "en_us"
				local manual_override = vim.b.spell_manual_override
				local prefix = manual_override and "!" or ""
				local lang_display = lang == "es" and "ES" or "EN"
				return prefix .. lang_display
			end,
